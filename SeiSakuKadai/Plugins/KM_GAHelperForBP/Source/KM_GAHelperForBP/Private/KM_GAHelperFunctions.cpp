...
void UKM_GAHelperFunctions::GetEffectLevel(UAbilitySystemComponent* AbilitySystem, TSubclassOf<class UGameplayEffect> Class, int& Level)
{
    if (AbilitySystem)
    {
        TArray<FGameplayEffectSpec> OutSpecCopies;
        AbilitySystem->GetAllActiveGameplayEffectSpecs(OutSpecCopies);
        for (auto spec : OutSpecCopies)
        {
            if (spec.Def->GetClass() == Class)
            {
                Level = spec.GetLevel();
                return;
            }
